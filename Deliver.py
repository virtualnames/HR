...
testers = ['OpenSSL', 'wolfSSL', 'GnuTLS', 'Cryptography', 'Bouncycastle', 'Go']
...
def main():
    ...
    for tester in testers:
        result_file = os.path.join(results_directory, f"{tester}.txt")
        all_converted[tester] = read_result_file(result_file)
        print(f"{tester}: Successfully converted {len(all_converted[tester])} certificates")

    common_converted = set.intersection(*all_converted.values())
    print(f"Number of certificates converted without errors: {len(common_converted)}")
